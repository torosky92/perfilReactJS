    ...
    return(
        <div className="Skill">
            <form className="container-fluid" onSubmit={(e) => e.preventDefault()}>
                <h1 className='mt-5' style={{marginTop:'10px'}}>{props.languages === 'SPANISH' ? 'SISTEMA OPERATIVO' : 'OS'}</h1>
                <div className="row mt-3">
                    {SystemOP.map((dato) => (
                        <ShowItem BoxIconColor={'black'} icon={dato.icons} title={dato.system}
                ...
                <h1 className='mt-5' style={{marginTop:'10px'}}>SOFTWARE</h1>
                <div className="row mt-3">
                    {SOFTWARE.map((dato) => (
                        <ShowItem BoxIconColor={'black'} icon={dato.icons} title={dato.software}
                            value={`${(YearsToNow(dato.year, dato.month, dato.day))} ${(props.languages === 'SPANISH' ? 'AÑOS' : 'YEARS')}`}
                            textValue={''} languages={props.languages}
                            library={[]} textLibrary={props.languages === 'SPANISH' ? 'LIBRERIAS' : 'LIBRARY'} hasLibrary={false}/>)
                    )}
                </div>
                <h1 className='mt-5' style={{marginTop:'10px'}}>{props.languages === 'SPANISH' ? 'LENGUAJE DE PROGRAMACIÓN': 'PROGRAMMING LANGUAGE'}</h1>
                <div className="row mt-3">
                    {ProgrammingLanguages.map((dato) => (
                        <ShowItem BoxIconColor={'black'} icon={dato.icons} title={dato.language}
                            value={`${(YearsToNow(dato.year, dato.month, dato.day))} ${(props.languages === 'SPANISH' ? 'AÑOS' : 'YEARS')}`}
                            textValue={props.languages === 'SPANISH' ? dato.level.Spanish : dato.level.English} languages={props.languages}
